      ...
      <div className='w-full flex justify-between items-center max-w-7xl mx-auto'>
        ...
        <div className='sm:hidden flex flex-1 justify-end items-center'>
          ...
          <div
            className={`${
              !toggle ? "hidden" : "flex"
            } p-6 glass-card-strong absolute top-20 right-0 mx-4 my-2 min-w-[140px] z-10 rounded-2xl border border-white/10 shadow-2xl`}
          >
            <ul className='list-none flex justify-end items-start flex-1 flex-col gap-4'>
              {navLinks.map((nav) => (
                <li
                  key={nav.id}
                  className={`font-poppins font-medium cursor-pointer text-[16px] transition-all duration-300 hover:text-gradient-primary ${
                    active === nav.title ? "text-gradient-primary" : "text-secondary"
                  }`}
                  onClick={() => {
                    setToggle(!toggle);
